...
如果你正在使用 Obsidian，Follow Builders 可以自动将每日/每周摘要归档到你的知识库中。
在初始化设置时，只需提供你的 Obsidian 库名称，工具会自动定位本地库路径，并将摘要内容追加到你的每日日记文件中（`Daily/YYYY-MM-DD.md`），位于 `## AI Builders Digest` 标题之下。
不需要安装任何第三方插件，完全基于 Obsidian 原生能力。
